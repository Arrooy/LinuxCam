...
CameraManager::CameraManager() : layerManager_(nullptr)
{
}
...
bool CameraManager::updateInput()
{
    if (!layerManager_)
    {
        common::logError("CameraManager::updateInput - LayerManager not set");
        return false;
    }
    ...
    for (auto& input : inWebcam_)
    {
        if (input->isRunning())
        {
            ...
            {
                common::logError("CameraManager::updateInput - Input image invalid size: %d "
                                  "x %d",
...
void CameraManager::updateCameraLayer(const std::shared_ptr<InputWebcam>& camera, std::unique_ptr<Image> newFrame)
{
    ...
    else
    {
        ...
        common::logInfo("CameraManager::updateCameraLayer - Created layer with name: '%s' "
                        "for device: %s",
...
void CameraManager::createOutputCameraOverlay(const std::shared_ptr<V4L2LoopbackWriter>& camera)
{
    ...
    {
        common::logWarn("CameraManager::createOutputCameraOverlay - No format available "
                        "for camera %s",
    ...
    common::logInfo("CameraManager::createOutputCameraOverlay - Created overlay for output "
                    "camera %s (%dx%d)",
    ...
    for (auto& output : outWebcam_)
    {
        if (output->isRunning())
        {
            ...
            if ((overlayLayer != nullptr) && overlayLayer->img)
            {
                ...
                {
                    common::logInfo("Cropped output for %s: region (%d,%d) %ux%u from "
                                    "composite %ux%u",
...
bool CameraManager::addCamera(const std::shared_ptr<Webcam>& camera /*camera*/)
{
...
bool CameraManager::removeCamera(const std::shared_ptr<Webcam>& camera /*camera*/)
{
    ...
    if (auto output = std::dynamic_pointer_cast<V4L2LoopbackWriter>(camera))
    {
        ...
        if (layerManager_)
        {
            ...
            if (overlayLayer != nullptr)
            {
                ...
                common::logInfo("CameraManager::removeCamera - Removed overlay for output "
                                "camera %s",
                                camera->getName().c_str());
            }
        }
...
bool CameraManager::updateCamera(const std::shared_ptr<Webcam>& camera /*camera*/)
{
    if (auto input = std::dynamic_pointer_cast<InputWebcam>(camera))
    {
...
std::vector<std::string> CameraManager::discoverAvailableVideoDevices()
{
    ...
    for (int i = 0; i < 64; ++i) // Check video0 through video63
    {
        ...
        if (common::fileExists(devicePath) && isDeviceUsable(devicePath))
        {
            ...
            common::logInfo("CameraManager::discoverAvailableVideoDevices - Found "
                            "unmanaged usable device: %s",
                            devicePath.c_str());
        }
    }
