...
std::vector<FaceLandmark> LandmarkConverter::pfldToWflw(const std::vector<FaceLandmark>& pfldLandmarks)
{
    ...
    const std::vector<std::pair<int, int>> wflwCurveSegments = {
...
std::vector<FaceLandmark> LandmarkConverter::wflwToPfld(const std::vector<FaceLandmark>& wflwLandmarks)
{
    ...
    const std::vector<std::pair<int, int>> pfldCurveSegments = {
        {0,  16}, // Jawline
        {17, 21}, // Right eyebrow
        {22, 26}, // Left eyebrow
        {27, 35}, // Nose
    ...
    for (const int missingIdx : missingIndices)
    {
        ...
        if (count > 0)
        {
            interpolated.emplace_back(
                FaceLandmark{missingIdx, math_utils::Point3D(sumX / count, sumY / count, sumZ / count)});
        }
        else
        {
            // Fallback: use center point or zero
            interpolated.emplace_back(FaceLandmark{missingIdx, math_utils::Point3D(0, 0, 0)});
        }
    }
