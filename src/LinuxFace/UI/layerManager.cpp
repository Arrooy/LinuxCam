...
void LayerManager::removeLayer(int layerNumber) {
    layers_.erase(std::remove_if(layers_.begin(), layers_.end(),
        [layerNumber](const Layer& l) { return l.getLayerNumber() == layerNumber; }), layers_.end());
}
...
std::vector<Layer>& LayerManager::getLayers() { return layers_; }
const std::vector<Layer>& LayerManager::getLayers() const { return layers_; }

Layer* LayerManager::getLayerByNumber(int layerNumber) {
    for (auto& l : layers_) {
        if (l.getLayerNumber() == layerNumber) return &l;
    }
    return nullptr;
}

void LayerManager::sortLayers() {
    std::sort(layers_.begin(), layers_.end(), [](const Layer& a, const Layer& b) {
...
bool LayerManager::isDirty() const {
    for (const auto& l : layers_) if (l.dirty) return true;
    return false;
}

void LayerManager::setDirty(bool dirty) {
    for (auto& l : layers_) l.dirty = dirty;
}
