    ...
    , io_binding_(nullptr)
{
    ...
    try
    {
        common::logInfo("OnnxDetector: Loading ONNX model from %s", onnx_model_path.c_str());
...
{
    Ort::TypeInfo inputTypeInfo = detector_session_->GetInputTypeInfo(0);
    ...
    int outputCount = 0 = detector_session_->GetOutputCount();
    names = detector_session_->GetOutputNames();
    for (int i = 0; i < outputCount; i++)
    {
        Ort::TypeInfo ouputTypeInfo = detector_session_->GetOutputTypeInfo(i);
        ...
        {
            common::logInfo("OnnxDetector::readModelInputSize - Detected shape: %d", output_dims[i]);
        }
    }
