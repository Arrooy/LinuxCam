...
PFLDDetector::PFLDDetector(const std::string& onnxModelPath) : OnnxDetector(onnxModelPath)
{
    ...
    if (output_node_names_str_.empty() || input_node_dims.size() < 4)
    {
        ...
        common::logError("Problem is %s, input dims: %s",
                         output_node_names_str.empty() ? "no outputs" : "invalid outputs",
...
void PFLDDetector::detect(const std::unique_ptr<Image>& image, Face& face)
{
    ...
    if (!alignedFace)
    {
        common::logError("Failed to crop aligned face image for MediaPipe landmarks "
                         "detection");
        return;
    }
    ...
    auto outputTensors = detector_session->Run(Ort::RunOptions{nullptr}, input_node_names.data(), &inputTensor, 1,
                                               output_node_names.data(), output_node_names_str.size());
    ...
    std::vector<math_utils::Point<>> alignedPts(numLandmarks);
    for (unsigned int i = 0; i < numLandmarks; ++i)
    {
        ...
        alignedPts[i] = {static_cast<long>(pt.x), static_cast<long>(pt.y)};
    }
...
void PFLDDetector::detectSimilar(const std::unique_ptr<Image>& image, Face& face)
{
    ...
    // 4. Run PFLD on aligned face
    Ort::Value inputTensor = this->transform(aligned_face);
    ...
    Ort::Value& landmarksTensor = output_tensors.at(outputIndex); // (1, 212)
    ...
    double invAffineM[6];
    if (!math_utils::invertAffine(affine.data(), inv_affineM))
    {
    ...
    std::vector<std::pair<double, double>> alignedPts(num_landmarks);
    for (unsigned int i = 0; i < numLandmarks; ++i)
    {
        double x = std::min(std::max(0.f, data[2 * i]), 1.0f);
        double y = std::min(std::max(0.f, data[2 * i + 1]), 1.0f);
        alignedPts[i] = {x * w, y * h};
    }

    auto unalignedPts = image_utils::transform_points_affine(aligned_pts, inv_affineM);
    std::vector<FaceLandmark> pfldLandmarks(num_landmarks);
    pfldLandmarks.reserve(numLandmarks);
    for (unsigned int i = 0; i < numLandmarks; ++i)
    {
        // Convert Point to Point3D (z=0)
        pfldLandmarks[i] = FaceLandmark{i, math_utils::Point3D(unaligned_pts[i].first, unaligned_pts[i].second, 0.0)};
    }
    face.loadNewFaceLandmarks(std::move(pfld_landmarks));
    Profiler::getInstance().stop("PFLDDetector", "detect landmarks");
}

static void PFLDDetector::detectOpenCv(const std::unique_ptr<Image>& image, Face& face)
{
    ...
    std::vector<cv::Point2f> srcStable = {five_pts_cv[0], five_pts_cv[1], five_pts_cv[2]};
    std::vector<cv::Point2f> dstStable = {template_cv[0], template_cv[1], template_cv[2]};
    ...
    common::logInfo("PFLDDetector: Input image size: %zu bytes", data_size);
    ...
    std::unique_ptr<Image> alignedFace = std::make_unique<Image>(data_size);
    std::memcpy(aligned_face->data(), alignedFaceRgb.data, dataSize);
    ...
    auto outputTensors = detector_session_->Run(Ort::RunOptions{nullptr}, input_node_names_.data(), &input_tensor, 1,
    ...
    std::vector<cv::Point2d> alignedPtsCv(num_landmarks);
    for (unsigned int i = 0; i < numLandmarks; ++i)
    {
    ...
    cv::transform(aligned_pts_cv, original_landmarks_cv, inverseAffine2x3);
