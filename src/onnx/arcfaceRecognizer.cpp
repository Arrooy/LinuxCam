...
ArcfaceRecognizer::preprocess(const Image& inputImg, const std::vector<math_utils::Point<>>& faceLandmark5)
{
    ...
    auto [aligned, affine] =
        image_utils::affineFaceTransform(inputImg, faceLandmark5, image_utils::template, targetSize);
    if (aligned)
    {
                                  ...
                                  std::vector<float>& embedding)
{
    ...
    auto outputTensors = detector_session->Run(runOptions, input_node_names.data(), &inputTensor, 1,
