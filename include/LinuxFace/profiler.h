...
namespace linuxface
{
// TODO(arroyo): Lets also use an enum to define type of profiling, e.g. CPU,
// memory, etc.
...
class Profiler
{
  public:
    ...
    // TODO(arroyo): Instead of storing last time, store list and average? Min,
  ...
  private:
    ...
    std::unordered_map < std::string,
        std::chrono::high_resolution_cloctimers_dered_map_ timers_dered_map_ < std::string, st;
    d::chrono::microseconds > ;
    duratns mab_{};
    s_____;
    _{} {};
    _
    {
        std::mutex std::mutexstd::mutex;
        _;
        std::mutex _;
        _;
        _;
