...
// TODO(arroyo): Still under evaluation.
namespace linuxface
{
...
class DepthImage : public Image
{
  public:
    ...
    std::unique_ptr<DepthImage> deepCopyDepth() const
    {
        ...
        if ((data() != nullptr) && (copy->data() != nullptr) && !size->empty())
        {
    ...
    void setNormals(const float* normalData, size_t normalDataSize)
    {
        ...
        common::logWarn("DepthImage::setNormals - Size mismatch. Expected %zu elements, "
                        "got %zu elements",
    ...
    void setConfidence(const float* confidenceData, size_t confidenceDataSize)
    {
        ...
        if (confidenceDataSize != expectedBytes)
        {
            ...
            common::logWarn("DepthImage::setConfidence - Expected: %zu bytes, Got: %zu bytes", expected_bytes,
                             confidenceDataSize);
    ...
    {
        if ((data() == nullptr) || size->empty() || info.width == 0 || info.height == 0)
        {
