...
class JPEGEncoder : public Encoder
{
  ...
  public:
    explicit JPEGEncoder(const ConfigBuilder& config)
    {
        ...
        if (!config.get("quality", quality_))
        {
        ...
        if (!config.get("pixelFormat", pixelFormat_))
        {
            common::logError("JPEGEncoder - Unable to load parameter pixelFormat");
        }

        if (!config.get("chrominance_subsampling", chrominance_subsampling_))
        {
    ...
    bool encode(const Image& srcImage, Image& outImage, unsigned long& compressedSize) override
    {
        ...
        if (static_cast<int>(srcImage.info.width) != width_ || static_cast<int>(srcImage.info.height) != height_)
        {
            common::logError("JPEGEncoder::encode - Image size mismatch: expected %dx%d, got "
                             "%lux%lu",
                             width_, height_, srcImage.info.width, srcImage.info.height);
            return false;
        }

        // Validate pixel format compatibility
        TJPF const sourcePixelFormat = srcImage.info.TJPixelFormat;
        if (sourcePixelFormat != pixelFormat_)
        {
            common::logWarn("JPEGEncoder::encode - Pixel format mismatch: expected %d, got "
                            "%d",
                            static_cast<int>(pixelFormat_), static_cast<int>(sourcePixelFormat));
        }

        // Validate buffer size
        const size_t expectedSize = width_ * height_ * tjPixelSize[sourcePixelFormat];
        ...
        const int pitch = width_ * tjPixelSize[sourcePixelFormat];

        const int tjStat =
            tjCompress2(c_handle_, srcImage.data(), width_, pitch, height_, sourcePixelFormat, &outImageData,
                        &outImageSize, chrominance_subsampling_, quality_, TJFLAG_NOREALLOC);

        if (tjStat != 0)
        {
            common::errnoLog("JPEGEncoder::encode - Compressor failed to compress!");
            common::errnoLog((const char*) tjGetErrorStr2(c_handle_));
            common::logError("JPEGEncoder::encode - Parameters: w=%d h=%d pitch=%d pf=%d "
                             "cs=%d q=%d",
                             width_, height_, pitch, static_cast<int>(sourcePixelFormat),
                             static_cast<int>(chrominance_subsampling_), quality_);
            return false;
        }
    ...
    unsigned long encodeSizeInBytes() override { return tjBufSize(width_, height_, chrominance_subsampling_); }
...
class BayerGBRGDecoder : public Decoder
{
  public:
    ...
    bool decodeHeader(Image& srcImage, unsigned long& rawNeededSize) override
    {
        ...
        // RGB output needs 3 times the space (3 bytes per pixel vs 1 byte per
        // pixel for Bayer)
...
class YUV422 : public Decoder
{
  public:
    ...
    bool genericDecode(const Image& srcImage, Image& outImage, F pxConvert)
    {
        ...
        const bool result = decodeYuv(srcImage, outImage, pxConvert);
        return result;
    }
  ...
  private:
    bool decodeYuv(const Image& yuvImage, Image& rgbImage, const ConversionFunct& funct)
    {
    ...
    __attribute__((always_inline)) void convertYuVtoRgb(uint8_t y, uint8_t u, uint8_t v, uint8_t* rgb)
    {
...
class PPMDecoder : public Decoder
{
  public:
    ...
    bool decodeHeader(Image& srcImage, unsigned long& rawNeededSize) override
    {
        ...
        if (srcImage.size() < headerEnd + pixelDataSize)
        {
            common::logError("PPMDecoder::decodeHeader - Invalid header. Not enough data for "
                             "pixel array");
            return false;
        }
