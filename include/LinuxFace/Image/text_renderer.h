...
struct TextRenderConfig
{
    ...
    explicit TextRenderConfig(std::string text, const Pixel& color = {255, 255, 255, 255}, int scale = 1)
        : text(std::move(text)), textColor(color), scale(scale)
    {
    }

    // Helper constructor for bounded text with wrapping
    TextRenderConfig(std::string text, int maxWidth, const Pixel& color = {255, 255, 255, 255}, int scale = 1)
        : text(std::move(text)), textColor(color), scale(scale), wrapMode(TextWrapMode::AUTO_WIDTH), maxWidth(maxWidth)
    {
    }
};
...
class TextRenderer
{
  public:
    ...
    static void drawTextWithBackground(Image& img, int x, int y, const std::string& text, const Pixel& textColor,
                                       const Pixel& bgColor, int scale = 1, bool center = false, int padding = 2);

    static void drawMultilineText(Image& img, int x, int y, const std::string& text, const Pixel& color, int scale = 1,
                                  int lineSpacing = 2);

    static void
    drawTextAligned(Image& img, int rectX, int rectY, int rectWidth, int rectHeight, const std::string& text,
  ...
  private:
    ...
    static void renderGlyph(Image& img, int x, int y, const unsigned char* glyph, const Pixel& color, int scale);
