...
WFLWPaths WFLWPaths::getDefault()
{
    ...
    paths.annotations_path =
        paths.base_path + "/WFLW_annotations/list_98pt_rect_attr_train_test/list_98pt_rect_attr_test.txt";
...
{
    return std::filesystem::exists(annotations_path) && std::filesystem::exists(images_path);
}
...
std::vector<int>
SimpleWFLWLoader::getSamplesByAttributes(bool normal_pose, bool normal_expression, bool normal_illumination,
                                         bool no_makeup, bool no_occlusion, bool is_clear) const
{
    ...
    for (int i = 0; i < static_cast<int>(samples_.size()); ++i)
    {
        ...
        if ((normal_pose == sample.isNormalPose()) && (normal_expression == sample.isNormalExpression())
            && (normal_illumination == sample.isNormalIllumination()) && (no_makeup == sample.hasNoMakeup())
            && (no_occlusion == sample.hasNoOcclusion()) && (is_clear == sample.isClear()))
        {
...
int SimpleWFLWLoader::countValidSamples(const std::string& annotations_path)
{
    ...
    while (std::getline(file, line))
    {
        ...
        while (ss >> temp && valid_fields < 203)
        { // 98*2 + 4 + 6 + 1 = 203 fields
            valid_fields++;
        }
...
loadImagesFromDirectory(const std::string& directory_path, const std::vector<std::string>& extensions, int max_images)
{
